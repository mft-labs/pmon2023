...
class ActiveThreadsMonitor(object):
    ...
    def _get_list_db_pools(self):
        logger = self.logger
        try:
            params = {'securetoken':self.securetoken,'qname':'ListDBpools'}
            resp = requests.get('http://%s:%d/queueWatch/queueWatcher'%(self.host_info['host'],int(self.host_info['port'])),params=params,cookies=self.cookies)
            soup = BeautifulSoup(resp.text,"lxml")
            table = soup.find_all('table')[0]
            list = {}
            ...
            for tr in table.find_all('tr'):
                ...
                list[str(row)] = columns
                row = row + 1
            return {'list_db_pools':list}
        except Exception as e:
            if logger != None:
                logger.error(traceback.format_exc())
        return None

    def get_list_db_pools(self,indexname):
        logger = self.logger
        results = self._get_list_db_pools()
        if results != None:
            db_pool = results['list_db_pools']
            row = 1
            while row < len(db_pool):
                info = db_pool[str(row)]
                d=datetime.utcnow()
                timestamp=calendar.timegm(d.utctimetuple())
                self.esdsl.writeDBPoolInfo(indexname,self.host_info['host'],info[0],info[1],info[2],info[3],info[4],info[5],info[6],info[7])
                row = row+1

    def get_thread_pool_info(self,indexname):
        logger = self.logger
        results = self.get_active_threads()
        if results != None:
            thread_pool = results['allqueues']
            row = 1
            while row < len(thread_pool):
                info = thread_pool[str(row)]
                d=datetime.utcnow()
                timestamp=calendar.timegm(d.utctimetuple())
                self.esdsl.writeThreadPoolInfo(indexname,self.host_info['host'],info[0],info[1],info[2],info[3],info[4],info[5],info[6])
                row = row+1
